...
- Treat the result as a render-only payload for the visual model, not narration, a character sheet, or a continuity report.
- Resolve every alternative into one physically coherent camera height, angle, crop, pose, and hand position. Do not emit "or", optional variants, or multiple camera choices unless the user explicitly requests alternatives.
- Convert history or anticipation such as "has just", "recently", "then", "about to", or "returned from" into present-frame visual evidence: current hand placement, damp hair, flushed skin, an open door, or relaxed posture.
- Preserve clearly adult body shape once with neutral fashion language. Prefer "full-busted hourglass silhouette", "fitted opaque cotton with natural tension, folds, and humidity-darkened areas", and "secure opaque neckline" over repeated breast, cleavage, chest, low-neckline, skin-fold, or curve emphasis.
- Prefer positive visible states such as "bare neckline" and "unadorned hands" over low-value absence lists. Keep an explicitly requested clean-output constraint such as no typography or watermark.
- Preserve visible adult decade identity with semantic wording such as "late-thirties adult"; do not flatten it to generic "adult". Keep minors and age-ambiguous subjects in their actual non-adult life stage.
...
The final text is a render-only payload for the visual model. Resolve one camera height, angle, crop, pose, and hand position; never leave alternatives such as "seated or standing". Turn "has just", "recently", "then", "about to", and other history into current visible evidence.
For a clearly adult subject, preserve body shape once with neutral fashion language: use "full-busted hourglass silhouette", fitted opaque fabric with natural tension and humidity-darkened areas, and a secure opaque neckline instead of repeated breast, cleavage, chest, skin-fold, or curve emphasis. Prefer "bare neckline" and "unadorned hands" to absence lists. Preserve an adult decade cue such as "late-thirties adult" rather than generic "adult".
