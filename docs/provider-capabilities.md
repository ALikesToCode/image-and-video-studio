...
- Image discovery uses the unified `GET /v1/models` catalog as its provider source of truth and admits only entries that explicitly declare image output through `supports_images`, `supports_image_output`, or output modalities. Provider-native NavyAI, LinkAPI, and NanoGPT catalogs overlay richer transport metadata without duplicating entries. A newly enabled unified image provider therefore becomes available without a Studio release, while text-only models remain excluded.
- Media IDs use bounded `provider:model` tags. Known native-provider tags are removed when forwarding to their provider routes; all other image providers retain the full ID and use unified `POST /v1/images/generations`. Automatic media routes are not accepted because an ambiguous retry could duplicate paid generation.
...
- Chat and standalone MultiLLM image submissions (including LinkAPI and GGUU) honor the configured tries for transient HTTP failures, including 524, using exponential delays and `Retry-After`. NavyAI retains single-attempt submission. A timeout retry can incur another generation charge. Authentication, validation, balance, and plan failures stop immediately. Accepted jobs stay on their polling path; polling or image-download failures do not resubmit generation. Retry delays over 60 seconds are reported for manual recovery.
